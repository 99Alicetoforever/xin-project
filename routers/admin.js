...
// console.log(__dirname);   //控制台输出：F:\new\routers  所以要返回上一级(../)
//console.log(__filename);
//console.log(process.cwd());

//__dirname:使用dirname变量获取当前模块文件所在目录的完整绝对路径
//multer是node js中处理multipart/form-data数据格式(主要用在上传功能中）的中间件
//dest是multer的参数，规定上传文件存放的位置 如果该参数省略，上传的文件将一直保存在内存中，永远不会写进磁盘。
...
router.post("/del",function(req,res){
    ...
    pool.getConnection(function(err,conn){
        if(err){
            ...
            conn.query("delete from taskApply where aid=?",[num],function (err, result) {
                conn.release();
                if(err){
                    msg.code=0;
                    msg.message="网络连接失败，请稍后重试...";
